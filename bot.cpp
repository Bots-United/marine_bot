...
bool b_observer_mode = FALSE;	// player is "invisible" for bot if TRUE
bool b_freeze_mode	 = FALSE;	// don't move if TRUE
bool b_botdontshoot  = FALSE;	// if TRUE bot will NOT press fire trigger, but "weapon methods" still runs
bool b_botignoreall  = FALSE;	// if TRUE ignore all, no enemies no combat

bool debug_aim_targets = FALSE;	// show aiming targets
bool debug_actions	 = FALSE;	// show current waypoint action
bool debug_stuck	 = FALSE;	// show current unstuck action
bool debug_waypoints = FALSE;	// show current waypoint the bot head towards
bool debug_weapons	 = FALSE;	// show weapons that the bot spawned with
...
//void BotSelectMainWeapon(bot_t *pBot);
...
//		killer_edict	= NULL;
		...
		if (main_weapon == -1)			// both most probably to spawninit() as well
			main_no_ammo	= true;
		else
			main_no_ammo	= false;
...
void bot_t::BotSpawnInit()
{
	...
	if (listenserver_edict)
		ALERT(at_console, "Bot Spawn Init\n");
#endif
	...
	wpt_origin = Vector(0, 0, 0);
	prev_wpt_origin = Vector(0, 0, 0);
	...
	f_duckjump_time = 0.0;
	f_dont_check_stuck = gpGlobals->time + 1.0;	// prevents false running of unstuck when spawning
	f_stuck_time = 0.0;
	changed_direction = 0;
...
#ifdef _DEBUG
	curr_aim_location = Vector(0, 0, 0);
	is_forced = false;
                ...
                const char *arg3, const char *arg4, const char *arg5)
{
	...
	else
	{
		...
		for (int ii=0; ii<ROUTE_LENGTH; ++ii) {
			pBot->point_list[ii] = -1;
		}
			...
			if (strcmp("item_claymore", item_name) == 0)
			{
				...
				if (BotEntityIsVisible(pBot, vecEnd))
				{
					...
					if (RANDOM_LONG(1, 100) > chance_to_miss)
					{
						...
						#ifdef _DEBUG
						//@@@@@@@@@@@@@
						ALERT(at_console, "***BotFindItem() - ignoring this claymore - like never seen\n");
						#endif
...
#ifdef _DEBUG
					//@@@@@@@@@@@@@@@@@
					//ALERT(at_console, "%s sees some grenade (dist %.2f)\n", pBot->name,
					//	(vecEnd - vecStart).Length());
#endif
					break;
				}
			}
...
void bot_t::BotDoMedEvac(float distance)
{
	...
	if (IsSubTask(ST_MEDEVAC_DONE))
	{
		...
		//@@@@@@@@@@
		//ALERT(at_console, "bot.cpp|doMedevac - EVAC DONE & all vars cleaned\n");
	...
	if (!IsSubTask(ST_MEDEVAC_ST))
	{
		...
		pEdict->v.v_angle.x = fabs(corpse_z - bot_z);
	...
	if (!IsSubTask(ST_MEDEVAC_H))
	{
		...
		//@@@@@@@@@@
		//ALERT(at_console, "bot.cpp|doMedevac - Trying HEAD\n");
	...
	if (!IsSubTask(ST_MEDEVAC_F))
	{
		...
		//@@@@@@@@@@
		//ALERT(at_console, "bot.cpp|doMedevac - Trying FEET\n");
...
#ifdef _DEBUG
				//@@@@@@@@
				ALERT(at_console, "this human teammate doesn't bleed -> no need to bandage him\n");
#endif
...
bool BotHealTeammate(bot_t *pBot)
{
	...
	if (((distance < 75) && (UTIL_IsOldVersion() == FALSE)) ||
		((distance < 50) && UTIL_IsOldVersion()))
	{
		...
		//char msg[256];
		//sprintf(msg, "BotHealTeammate | bot medic team %d  --- patient team %d\n", bot_team, player_team);
		//UTIL_DebugDev(msg, -100, -100);

		//if (bot_team == TEAM_NONE || player_team == TEAM_NONE)
		//	UTIL_DebugDev("BotHealTeammate | !! IMPORTANT !! - check the line above -> TEAM_NONE\n", -100, -100);
		...
		if (bots_head > patients_head)
			pEdict->v.v_angle.x = fabs(patients_head - bots_head);
...
#ifdef _DEBUG
			//@@@@@@@@@@@@@@@@@@
			ALERT(at_console, "facing my patient (run only once per treatment!!!)\n");
#endif
		...
		if (pBot->IsSubTask(ST_TREAT))
		{
			...
			pBot->f_medic_treat_time = gpGlobals->time +
				RANDOM_FLOAT(2.5, 3.5) + delay[skill];
		...
		else if (pBot->IsSubTask(ST_HEAL))
		{
			...
			{
				pBot->f_medic_treat_time = gpGlobals->time +
					RANDOM_FLOAT(1.0, 2.0) + delay[skill];


#ifdef _DEBUG
				//@@@@@@@@@@@@@@@@
				char msg[256];
				sprintf(msg, "(!!!bot.cpp->HealTeammate()) Additional BANDAGING time to heal <%s>!!!\n",
					STRING(pWounded->v.netname));
...
#ifdef _DEBUG
		//@@@@@@@@@@@@@@@@@@
		ALERT(at_console, "***CAN'T HELP --> patient too far!\n");
#endif
	...
	else if ((distance < 250) && (pBot->pEdict->v.flags & FL_PRONED))
	{
...
void bot_t::BotPressFireToRespawn()
{
	...
	if ((main_weapon != -1) && (backup_weapon != -1))
	{
		forced_usage = USE_MAIN;
		// flags must be set again after death
		main_no_ammo = FALSE;
		backup_no_ammo = FALSE;
	}
	// have main weapon AND no backup weapon so use main
	else if ((main_weapon != -1) && (backup_weapon == -1))
	{
		forced_usage = USE_MAIN;
		main_no_ammo = FALSE;
		backup_no_ammo = TRUE;
...
void BotCheckSkillSystem(void)
{
	skill_system = CVAR_GET_FLOAT("sv_useskills");  // which system is set?

	checked_skillsystem = TRUE;
}
...
* if all is fine then the bot do change to main weapon
*/
void bot_t::BotSelectMainWeapon()
{
	if ((main_no_ammo == FALSE) && (f_shoot_time < gpGlobals->time) &&
		(f_use_clay_time < gpGlobals->time) &&
		(current_weapon.iId != main_weapon) && (weapon_action == W_READY) &&
		((clay_action == ALTW_NOTUSED) || (clay_action == ALTW_USED)))
	{
		...
		if (debug_actions)
			ALERT(at_console, "%s is trying to take back the MAIN weapon\n",
...
void BotSelectBackupWeapon(bot_t *pBot)
{
		...
		((pBot->clay_action == ALTW_NOTUSED) || (pBot->clay_action == ALTW_USED)))
	{
		...
		if (debug_actions)
			ALERT(at_console, "%s is trying to take back the BACKUP weapon\n",
...
void BotSelectKnife(bot_t *pBot)
{
		...
		((pBot->clay_action == ALTW_NOTUSED) || (pBot->clay_action == ALTW_USED)))
	{
		...
		if (debug_actions)
			ALERT(at_console, "%s is trying to take the KNIFE\n",
				STRING(pBot->pEdict->v.netname));
	}
}

/*
* routes we need to handle targeting a pItem entity
*/
void bot_t::FacepItem(void)
{
	...
	else
	{
		...
		//@@@@@@@@@@@@@@@@@@@@@@@
		//#ifdef _DEBUG
		//ALERT(at_console, "FacepItem successfully DONE \n");
		//#endif
...
*/
void bot_t::TargetAimWaypoint(void)
{
	...
	if (local_wpt == -1)
	{
		...
		if (debug_aim_targets || debug_waypoints)
			ALERT(at_console, msg);
	...
	if (IsSubTask(ST_AIM_FACEIT))
	{
		...
		if (IsTask(TASK_PRECISEAIM))
		{
			...
			{
				Vector v_aim = waypoints[curr_aim_index].origin -
					waypoints[curr_wpt_index].origin;
				Vector aim_angles = UTIL_VecToAngles(v_aim);
			...
			else
			{
				...
				//@@@@@@@@@@@@@@@@@@@@@@@
				#ifdef _DEBUG
				//ALERT(at_console, "SubTask FACEIT successfully DONE \n");
				#endif
	...
	if (IsSubTask(ST_AIM_SETTIME))
	{
		...
		if (aim_count > 1)
		{
			...
			if (bot_team == TEAM_RED)
				the_time = waypoints[local_wpt].red_time;
			else
				the_time = waypoints[local_wpt].blue_time;
			...
			if (the_time == 0.0)
				the_time = RANDOM_LONG(2.0, 5.0);
				
			// see how many times the bot should change aim waypoint
			// (ie the longer wait time the more changes)
		...
		// otherwise target the only aim waypoint and keep it the whole wait time
		else if (aim_count == 1)
		{
		...
		// we can safely break this method here in this task, because there's nothing more
		// to be done and we save some CPU time (that if statement below)
		return;
	}
...
void bot_t::BotWaitHere()
{
	...
	if (IsSubTask(ST_DOOR_OPEN) && waypoints[local_wpt].flags & (W_FL_DOOR | W_FL_DOORUSE))
	{
		...
		{
			if (debug_actions)
				ALERT(at_console, "***BotWait | DoorWptActions | !!! NO DOORS !!! -> leaving\n");
		...
		if (FInNarrowViewCone(&v_door, pEdict, 0.95))
		{
			...
			if ((tr.flFraction < 1.0) && doors == TRUE)
			{
				wpt_wait_time = gpGlobals->time + 1.0;
			...
			else
			{
				...
				//@@@@@@@@@@@@@@2
				//ALERT(at_console, "***BotWait | DoorWptActions | Doors opened -> leaving\n");
			}
		}
	...
	// is bot waiting at claymore waypoint
	else if (waypoints[local_wpt].flags & W_FL_MINE)
	{
		...
		else if (clay_action == ALTW_PREPARED)
		{
			...
			if (WaypointReturnPriority(local_wpt, bot_team) == 1)
			{
				placed_as_goal = TRUE;
			}
			...
			if (IsSubTask(ST_AIM_DONE))
			{
				...
				#ifdef _DEBUG
				//@@@@@@@@@@@@@@@@@@@@@@
				ALERT(at_console, "CLAYMORE - already facing desired direction - time to USE it\n");
				#endif
			...
			else
			{
				...
				#ifdef _DEBUG
				//@@@@@@@@@@@@@@@@@@@@@@
				ALERT(at_console, "CLAYMORE -> in process of facing desired direction\n");
				#endif
	...
	else if ((waypoints[local_wpt].flags & W_FL_FIRE) && (IsSubTask(ST_AIM_DONE)))
	{
		...
		if (WaypointReturnPriority(local_wpt, bot_team) == 0)
			dont_shoot = true;
		// if the waypoint has priority == 1 then don't check for object's existence
		// ie. the bot will always fire
		else if (WaypointReturnPriority(local_wpt, bot_team) == 1)
			dont_shoot = false;
		else
		{
			// we will be definetely firing if the the folloing method doesn't return false
			dont_shoot = false;

			// check if there is any breakable object
			if (UTIL_CheckForwardForBreakable(pEdict) == FALSE)
			{
				...
				{
					// there's completely nothing so we store the last time of waiting and we
					// return the bot back to normal navigation
					dont_shoot = true;
					wpt_wait_time = gpGlobals->time;
					...
					//@@@@@@@@@@@@@@@@@@@@@@@
					#ifdef _DEBUG
					ALERT(at_console, "bot.cpp ->WAIT by the SHOOT wpt --- completely NO OBJECT around !!!!!\n");
					#endif
				...
				// we found something somewhere around the bot so the bot has to ignore
				// aim waypoints to successfully face the object
				else
				{
					...
					//@@@@@@@@@@@@@@@@@@@@@@@
					#ifdef _DEBUG
					ALERT(at_console, "bot.cpp ->WAIT by the SHOOT wpt --- there's SOME OBJECT in surrounding! YAY!\n");
					#endif



					SetTask(TASK_IGNOREAIMS);
					// we just need to set some wait time so the bot won't
					// run away if there are still some breakables in the vicinity
		...
		if (dont_shoot == false)
		{
			// we need to keep increasing the wait time so the bot won't
			// run away if there is still some breakable object
			wpt_wait_time = gpGlobals->time + 1.0;
			...
			if (current_weapon.iClip == 0 && current_weapon.iAmmo1 != 0)
			{
				...
				f_reload_time = gpGlobals->time + 0.6;
				dont_shoot = true;
			}
			// if the bot is sniper he should zoom in
			// (that's due to stupid feature of feature of FA3.0
			// where the no zoom accuracy of sniper rifle is really awful)
			...
			// can the bot at all? (ie. is the weapon ready to fire)
			if (dont_shoot == false)
			{
				// is weapon firemode NOT auto so press "fire" button
				if ((f_shoot_time <= gpGlobals->time) && (current_weapon.iFireMode != 4))
				{
					pEdict->v.button |= IN_ATTACK;
				...
				{
					pEdict->v.button |= IN_ATTACK;
					// store the time even here so the bot know when did he shoot last
					f_shoot_time = gpGlobals->time;
				}
			}
		}
	...
	else if ((waypoints[local_wpt].flags & W_FL_PRONE) && !(pEdict->v.flags & FL_PRONED))
		UTIL_GoProne(this);
	
	if (debug_actions)
		ALERT(at_console, "Waiting at curr_wpt=%d | prev_wpt[0]=%d | prev_wpt[1]=%d\n",
		curr_wpt_index + 1, prev_wpt_index.get() + 1,
		prev_wpt_index.get(1) + 1);
	...
	if (forced_usage == USE_MAIN && (current_weapon.iId != main_weapon))
		BotSelectMainWeapon();
	else if (forced_usage == USE_BACKUP && (current_weapon.iId != backup_weapon))
...
#ifdef _DEBUG
		if (debug_actions)
			ALERT(at_console, "*** aiming marks were cleared (curr wpt index %d)\n", curr_wpt_index+1);
#endif
}
...
*/
...
#ifdef _DEBUG
		ALERT(at_console, "***bot %s has been promoted\n", STRING(pEdict->v.netname));
#endif
	}
	...
	// b_botignoreall completely skip this part -> we ignore everything
	if ((f_sound_update_time <= gpGlobals->time) && (b_botignoreall == FALSE) &&
		((pBotEnemy == NULL) || ((pBotEnemy) && (f_bot_wait_for_enemy_time))) &&
		(pEdict->v.movetype != MOVETYPE_FLY))
	{
		int ind;
		edict_t *pPlayer;
	...
	// do any speed adjustments only if bot is NOT proned or crouched (ie bot is moving fast)
	if (!(pEdict->v.flags & (FL_PRONED | FL_DUCKING)))
	{
		// slow down if pitch is positive ie looking down - for example going down the hill
		// otherwise don't slow down

		// don't move while turning a lot
		if (((pitch_degrees >= 50) && (pEdict->v.v_angle.x > 0)) || (yaw_degrees >= 50))
			move_speed = SPEED_NO;
		// slow down if we aren't turning that much
		else if (((pitch_degrees >= 25) && (pEdict->v.v_angle.x > 0)) || (yaw_degrees >= 25))
			move_speed = SPEED_SLOWER;
		// if we are turning only a bit use half of current speed
		else if (((pitch_degrees >= 10) && (pEdict->v.v_angle.x > 0)) || (yaw_degrees >= 10))
			move_speed = SPEED_SLOW;
	}
	...
	else
	{
		...
		if (b_botignoreall == FALSE)
		{
			...
			if (clay_action == ALTW_GOALPLACED)
			{
				if (f_bot_find_enemy_time <= gpGlobals->time)
				{
					...
					if (pBotEnemy != NULL)
					{
						// in most of the time detonate the mine even if bot isn't
						// in safe distance
		...
		if (pBotEnemy == NULL)
		{
			// does the bot use any sniper rifle AND has scope on
			if ((pEdict->v.fov != NO_ZOOM) && (f_shoot_time < gpGlobals->time) &&
				(pItem == NULL))
			{
				if (UTIL_IsSniperRifle(current_weapon.iId))
				{
					pEdict->v.button |= IN_ATTACK2;		// scope off
					f_shoot_time = gpGlobals->time + 1.0;	// time to take effect

					if (g_debug_bot_on)
					{
						extern edict_t *pRecipient;
						ClientPrint(pRecipient, HUD_PRINTNOTIFY, "No Enemy - Scope off\n");
					}
				}
			}
				...
				(f_bot_see_enemy_time > 0) && (f_bot_see_enemy_time + 0.1 <= gpGlobals->time))
			{
				...
				if (WaypointFindLastPatrol(this))
				{
					...
					if (debug_paths)
						ALERT(at_console, "PATROL path return flag cleared (Bot gets back to patrol)\n");
				}
			}
		}

		// used the bot a grenade recently so check if he still has any grenade left
		if ((grenade_action != ALTW_USED) && (grenade_time + 1.0 > gpGlobals->time) && (grenade_slot != -1) &&
...
#ifdef _DEBUG		
			//@@@@@@@@@@@
			ALERT(at_console, "(>>bot.cpp<<) both grenades used --> (USED flag set)!!!\n");
#endif
		}
			...
			if (IsTask(TASK_HEALHIM))
			{
				...
				else
				{
						...
						BotMedicGetToPatient(this))
					{
						...
						{
							//ALERT(at_console, "***(bot)treatment in progress\n");
						}
						#endif
					}
...
#ifdef _DEBUG
						//@@@@@@@@@@@@@@@@@@
						ALERT(at_console, "***CAN'T get to patient --> clearing variables\n");
#endif
					}
				}
			}
		...
		else if ((f_pause_time > gpGlobals->time) ||
			(f_medic_treat_time > gpGlobals->time))
		{
			f_waypoint_time = gpGlobals->time;
			f_dont_look_for_waypoint_time = gpGlobals->time;
		...
		// look if the bot is still doing bandage treatment
		else if ((bandage_time != -1.0) && (bandage_time > gpGlobals->time))
		{
			f_waypoint_time = gpGlobals->time;
			f_dont_look_for_waypoint_time = gpGlobals->time;
			...
			// if NOT proned or currently changing Stance then crouch for cover
			if (!(pEdict->v.flags & FL_PRONED) &&
				(f_stance_changed_time < gpGlobals->time))
				pEdict->v.button |= IN_DUCK;
		}
		...
		else if (IsTask(TASK_PARACHUTE_USED) && BotCheckMidAir(pEdict))
		{
			...
			// if the bot has no waypoint to head to then don't move
			// (otherwise the bot will try to land by his waypoint)
			if (curr_wpt_index == -1)
				move_speed = SPEED_NO;
		...
		else if (pBotUser != NULL)
		{
			if (BotFollowTeammate(this) == TRUE)
			{
				...
				if ((pMaster->v.flags & FL_PRONED) && !(pEdict->v.flags & FL_PRONED))
					UTIL_GoProne(this);

				// is bot proned but master isn't so stand up
				if ((pEdict->v.flags & FL_PRONED) && !(pMaster->v.flags & FL_PRONED))
					UTIL_GoProne(this);

				// has bot set this flag during waypoint navigation so clear it
		...
		else if (f_reload_time > gpGlobals->time)
		{
			...
			{
				f_reload_time = gpGlobals->time + 0.2;

				#ifdef _DEBUG
				if (debug_actions)
					ALERT(at_console, "(bot.cpp|botthink()|reload_time) weapon still not loaded -> reload time has been extended\n");
				#endif
			}
		...
		// is the bot still using bipod after combat
		else if (IsTask(TASK_BIPOD))
		{
			// stop using bipod if snipe and bipod handling time are over
			if ((sniping_time + 3.5 < gpGlobals->time) &&
				(f_bipod_try_time < gpGlobals->time))
			{
				FakeClientCommand(pEdict, "bipod", NULL, NULL);

				// set some time for next handling
				f_bipod_try_time = gpGlobals->time + 3.0;
			}
		...
		else if (IsTask(TASK_FIRE))
		{
			if (IsSubTask(ST_AIM_DONE))
			{
				...
				if (WaypointReturnPriority(curr_wpt_index, bot_team) == 1)
				{
					// just fire
					pEdict->v.button |= IN_ATTACK;
					f_shoot_time = gpGlobals->time;
				...
				else
				{
					...
					// has to be here even if the bot didn't really shoot
					// otherwise the bot might want to use the shoot waypoint again
					f_shoot_time = gpGlobals->time;
				}
		...
		else if (IsTask(TASK_BREAKIT))
		{
			...
			if ((pBreakable == NULL) || (pBreakable->v.health <= 0) ||
				(pBreakable->v.spawnflags & SF_BREAK_TRIGGER_ONLY))
			{
			...
			else
			{
				...
				else
				{
					...
					else if ((f_shoot_time <= gpGlobals->time) && (current_weapon.iFireMode == 4))
						pEdict->v.button |= IN_ATTACK;
				}
			}
		}
		...
		else if ((wpt_action_time > gpGlobals->time) &&
			(bot_spawn_time + 0.5 < gpGlobals->time))
		{
			int do_action;
			...
			{
				if (debug_actions)
					ALERT(at_console, "Bot has no item yet\n");

				do_action = UTIL_DoWaypointAction(this);
				...
				if (do_action == -1)
				{
					...
					if (debug_actions)
						ALERT(at_console, "NO item has been found, but hitting the use key anyway -- leaving wpt\n");
			...
			else if (f_face_item_time > gpGlobals->time)
			{
				...
				{
#ifdef NDEBUG
					UTIL_DebugInFile("BOT|W_A|F|PI_N\n");
#endif

#ifdef _DEBUG
					char msg[80];
					sprintf(msg, "BotThink() | Wpt_Action_Time | facing item | Item pointer is NULL | on map %s\n",
			...
			else
			{
				...
				{
#ifdef NDEBUG
					UTIL_DebugInFile("BOT|W_A|U|PI_N\n");
#endif

#ifdef _DEBUG
					char msg[80];
					sprintf(msg, "BotThink() | Wpt_Action_Time | using item | Item pointer is NULL | on map %s\n",
						STRING(gpGlobals->mapname));
				...
				else if (!IsSubTask(ST_BUTTON_USED) && !IsTask(TASK_USETANK))
				{
					...
					if (debug_actions)
						ALERT(at_console, "button used only once\n");
				}
...
#ifdef _DEBUG
						if (debug_actions)
							ALERT(at_console, "(bot)using TANK\n");
#endif
					}
		...
		else
		{
			...
			if ((gpGlobals->time > bot_spawn_time + 1.0) &&
				(gpGlobals->time < bot_spawn_time + 2.0))
			{				
				// is the bot NOT holding any of "single firemode" weapons
				// so change the firemode to semi (single shot)
				if ((current_weapon.iFireMode != FM_NONE) &&
					(current_weapon.iFireMode != FM_SEMI))
				{
					UTIL_ChangeFireMode(this, FM_SEMI, test_weapons);
				}
			}
			...
			if (bot_spawn_time + 2.5 < gpGlobals->time)
			{
				...
				if ((weapon_status & WA_ALLOWSILENCER) &&
					!(weapon_status & WA_SILENCERUSED) &&
					UTIL_IsStealth(pEdict) &&
					UTIL_CanMountSilencer(this))
				{
					// tell the bot to mount the silencer as soon as possible
					weapon_status |= WA_MOUNTSILENCER;
				}
				...
				if ((weapon_status & WA_MOUNTSILENCER) &&
					!(weapon_status & WA_SILENCERUSED) &&
					(weapon_action == W_READY))
				{
...
#ifdef _DEBUG
						ALERT(at_console, "***Mounting the silencer\n");
#endif
					}
					else
					{
...
#ifdef _DEBUG
						ALERT(at_console, "***Silencer can't be mounted - Resetting\n");
#endif
					}
				}
				...
				if (!IsNeed(NEED_GOAL) && !IsNeed(NEED_GOAL_NOT))
				{
					...
					{
						if (IsNeed(NEED_GOAL))
							ALERT(at_console, "***the bot decided to go for map objectives\n");
						else if (IsNeed(NEED_GOAL_NOT))
							ALERT(at_console, "***the bot decided NOT to go for map objectives\n");
					}
#endif
				}
			}
			...
			if ((f_reload_time < gpGlobals->time) &&
				(f_reload_time + 1.0 > gpGlobals->time))
			{
				SetTask(TASK_CHECKAMMO);

				if (debug_actions)
					ALERT(at_console, "Check ammunition after we have reloaded the weapon\n");
			}

			// or is some time after last ammo reserves checking
			else if (check_ammunition_time + 25.0 < gpGlobals->time)
			{
				SetTask(TASK_CHECKAMMO);

				if (debug_actions)
					ALERT(at_console, "Regular ammunition check is needed\n");
			}

			// reload second m11 in FA25
			if ((current_weapon.iId == fa_weapon_m11) &&
				(current_weapon.iAttachment == 0) && (current_weapon.iAmmo2 > 0) &&
				(pBotEnemy == NULL) && (f_shoot_time < gpGlobals->time) &&
				(f_reload_time < gpGlobals->time) &&
				(bot_spawn_time + 5.0 < gpGlobals->time) &&
				(wpt_action_time < gpGlobals->time))
			{
				...
				if (debug_actions)
					ALERT(at_console, "Going to reload current weapon\n");
			}

			// reload weapon if is empty clip AND have magazine AND no enemy AND
			// NOT currently firing AND NOT currenly reloading
			// AND some time after spawn AND NOT doing any waypoint based action
			if ((current_weapon.iClip == 0) && (current_weapon.iAmmo1 >= 1) &&
				(pBotEnemy == NULL) && (f_shoot_time < gpGlobals->time) &&
				(f_reload_time < gpGlobals->time) &&
				(bot_spawn_time + 5.0 < gpGlobals->time) &&
				(wpt_action_time < gpGlobals->time))
			{
				...
				f_reload_time = gpGlobals->time + 3.0;	// secures one run only
				move_speed = SPEED_SLOW;				// slow down while reloading
			...
			else
			{
					...
					(main_no_ammo == FALSE))
				{
					...
					if (debug_actions)
						ALERT(at_console, "MAIN weapon completely out of ammo (no magazines)\n");
				}
				// otherwise try to use main weapon at all cost
				else if ((forced_usage != USE_MAIN) ||
					...
					(backup_no_ammo == FALSE))
				{
					...
					if (debug_actions)
						ALERT(at_console, "BACKUP weapon completely out of ammo (no magazines)\n");
				}
				...
				if ((f_dont_look_for_waypoint_time < gpGlobals->time) &&
					(num_waypoints != 0))
				{
				...
				if (found_waypoint == FALSE)
				{
						...
						BotCantMoveForward( this, &tr ))
					{
						...
						//@@@@@@@@@@@@@@@@@@@@@@@@@
						//#ifdef _DEBUG
						//ALERT(at_console, "Trace returns %.2f\n", tr.flFraction);
						//#endif
	...
	if (curr_wpt_index != -1)
	{
		...
		if ((waypoints[curr_wpt_index].flags & W_FL_CROUCH) && !(pEdict->v.flags & FL_PRONED))
			pEdict->v.button |= IN_DUCK;
	...
	if (IsTask(TASK_DEATHFALL) && (f_check_deathfall < gpGlobals->time))
	{
		...
		if ((pBotEnemy != NULL) && (IsDeathFall(pEdict) || IsForwardBlocked(this)))
		{
			move_speed = SPEED_NO;
			SetDontMoveTime(1.0);
			f_dont_check_stuck = gpGlobals->time + 1.5;
		}
	...
	if ((weapon_action != W_READY) && (weapon_action != W_INRELOAD))
	{
	...
	if ((moved_distance <= 1.0) && (prev_speed != SPEED_NO) && (pEdict->v.movetype != MOVETYPE_FLY) &&
		(f_face_wpt < gpGlobals->time) && (f_dont_check_stuck < gpGlobals->time))
	{
		...
		if ((strcmp(STRING(tr.pHit->v.classname), "func_breakable") == 0) &&
			(tr.pHit->v.takedamage == DAMAGE_YES))
		{
			move_speed = SPEED_NO;
			pEdict->v.button |= IN_ATTACK;
...
#ifdef _DEBUG
			if (debug_stuck)
				ALERT(at_console, "[[Stuck]] on breakable object, just shoot it \n");
#endif
		}

		// if the bot is beeing "used" AND is very close to user so just stop
		else if ((pBotUser != NULL) &&
			((pEdict->v.origin - pBotUser->v.origin).Length() < 150))
		{
			move_speed = SPEED_NO;

#ifdef _DEBUG
			if (debug_stuck)
				ALERT(at_console, "[[Stuck]] while following user, just stopping -- quite close\n");
#endif
		}

		// can the bot jump (ie has enough stamina to jump and not proned)
		// and can jump onto something?
		else if (!IsTask(TASK_NOJUMP) && !(pEdict->v.flags & FL_PRONED) &&
			BotCanJumpUp(this))
		{
			pEdict->v.button |= IN_JUMP; // jump up and move forward
			pEdict->v.button |= IN_DUCK; // use duckjump to get bot over obstacle easier
			f_duckjump_time = gpGlobals->time + 1.0; // keep the "courch" key pressed

#ifdef _DEBUG
			if (debug_stuck)
				ALERT(at_console, "[[Stuck]] bot can jump over it\n");
#endif
		}
...
#ifdef _DEBUG
			if (debug_stuck)
				ALERT(at_console, "[[Stuck]] bot can DUCKJUMP over it\n");
#endif
		}
		// can the bot duck under something?
		else if (BotCanDuckUnder(this))
		{
			// if NOT proned (if bot is already proned pressing "crouch" will break the prone
			// which we won't)
			if (!(pEdict->v.flags & FL_PRONED))
...
#ifdef _DEBUG
			if (debug_stuck)
				ALERT(at_console, "[[Stuck]] bot can crouch under it\n");
#endif
		}
...
#ifdef _DEBUG
			if (debug_stuck)
				ALERT(at_console, "[[Stuck]] can strafe LEFT\n");
#endif
		}
...
#ifdef _DEBUG
			if (debug_stuck)
				ALERT(at_console, "[[Stuck]] can strafe RIGHT\n");
#endif
		}
		else
		{
		...
		if (f_stuck_time + 1.0 > gpGlobals->time)
		{
			...
			if (pEdict->v.flags & FL_PRONED)
			{
				...
				SetStanceByte(this, BOT_STANDING);

				// don't go prone for some time
				f_cant_prone = gpGlobals->time + 1.5;
				RemoveTask(TASK_GOPRONE);

#ifdef _DEBUG
				if (debug_stuck)
					ALERT(at_console, "[[Stuck]] when PRONED --> standing up\n");
#endif
			}
			...
			else if (IsEntityInSphere("bodyque", pEdict, 50))
			{
				...
				if (debug_stuck)
				{
					HudNotify("[[Stuck]] on DEAD BODY --> waiting till it disappers\n");
					UTIL_DebugDev("Stuck on DEAD BODY --> waiting till it disappers\n", -100, -100);
				}
#endif
			}
			else
...
#ifdef _DEBUG
				if (debug_stuck)
					ALERT(at_console, "BOT IS COMPLETELY STUCK - trying turn to random direction (# of tries before committing suicide %d)\n", changed_direction);
#endif
			}
...
#ifdef _DEBUG
				if (debug_stuck)
					ALERT(at_console, "[[Stuck]] COMPLETELY & NO WAY TO UNSTUCK --> commit SUICIDE\n");
#endif
	...
	// do we lost some health AND is it time to bandage the bot
	// AND is already some time left from last bandage treatment AND pause time is over
	else if ((bot_health < bot_prev_health) && (bandage_time != -1.0) && (bandage_time + 1.0 < gpGlobals->time) &&
		(f_pause_time < gpGlobals->time) && !IsTask(TASK_DROWNING))
	{
		...
		{
			// has the bot some bandages
			if (bot_bandages > 0)
			{
#ifdef _DEBUG
				ALERT(at_console, "***Start bandage treatment\n");
#endif
				// so use bandages (ie start bandage treatment)
				FakeClientCommand(pEdict, "usebandage", NULL, NULL);

				// set some time to successfully finish the treatment
				bandage_time = gpGlobals->time + 4.5;
			...
			else if ((!(bot_fa_skill & (FAID | MEDIC))) &&
				((speak_time + RANDOM_FLOAT(2.0, 5.0)) < gpGlobals->time))
			{
				...
				{
					f_pause_time = gpGlobals->time + RANDOM_FLOAT(1.5, 3.5);

					if (debug_actions)
						ALERT(at_console, "Waiting if medic comes\n");
				}
			}
		}
	}
	...
	else if ((chute_action_time > 1.0) && !IsTask(TASK_PARACHUTE_USED) && (pEdict->v.flags & FL_ONGROUND))
	{
		...
		if (debug_actions)
		{
			extern edict_t *listenserver_edict;
			if (listenserver_edict)
				ClientPrint(listenserver_edict, HUD_PRINTNOTIFY, "Resetting parachute - not in air\n");
		}
	}
		...
		(chute_action_time <= gpGlobals->time) && (bot_spawn_time + 1.0 < gpGlobals->time))
	{
		...
		if (chute_action_time < 1.0)
		{
			...
			if (debug_actions)
			{
				extern edict_t *listenserver_edict;
				if (listenserver_edict)
					ClientPrint(listenserver_edict, HUD_PRINTNOTIFY,
						"Testing if really in mid-air - low object falls prevention\n");
			}
		}
		...
		if ((chute_action_time > 1.0) && (chute_action_time < gpGlobals->time))
		{
			...
			if (debug_actions)
			{
				...
				if (listenserver_edict)
					ClientPrint(listenserver_edict, HUD_PRINTNOTIFY,
						"Testing if really in mid-air - yes realy in air -> using parachute\n");
			}
		}
	}
		...
		(pEdict->v.movetype != MOVETYPE_FLY) && (pEdict->v.flags & (FL_ONGROUND | FL_INWATER)))
	{
		...
		if (debug_actions)
		{
			extern edict_t *listenserver_edict;
			if (listenserver_edict)
				ClientPrint(listenserver_edict, HUD_PRINTNOTIFY, "Back on ground - throwing parachute away\n");
		}
	}
	...
	else if (IsNeed(NEED_RESETPARACHUTE) && chute_action_time < gpGlobals->time)
	{
		...
		if (debug_actions)
		{
			extern edict_t *listenserver_edict;
			if (listenserver_edict)
				ClientPrint(listenserver_edict, HUD_PRINTNOTIFY, "***finalizing parachute reset\n");
		}
	}
	...
	{
		if (debug_actions)
			ALERT(at_console, "Activating/detonating claymore mine\n");
	...
	// is the bot forced to evade a claymore
	...
	// is the bot forced to go prone and NOT proned yet, go prone then
	else if (IsTask(TASK_GOPRONE) && !(pEdict->v.flags & FL_PRONED))
	{
		UTIL_GoProne(this);
	}
	...
	else if (IsTask(TASK_USETANK))
	{
		...
		if ((f_face_item_time < gpGlobals->time) &&
			(pEdict->v.viewmodel != 0) && (pEdict->v.health > 0))
		{
			// stop using tank
			RemoveTask(TASK_USETANK);
...
#ifdef _DEBUG
			if (debug_actions)
				ALERT(at_console, "(bot)stopped using TANK\n");
#endif
		}
...
#ifdef _DEBUG
					if (debug_actions)
						ALERT(at_console, "(bot)stopped using TANK (the use time is over)\n");
#endif
				}
...
#ifdef _DEBUG
		if (debug_actions)
			ALERT(at_console, "<%s>Use waypoint action time over - (actions cleared)\n", name);
#endif
...
#ifdef _DEBUG
		if (debug_actions)
			ALERT(at_console, "<%s>Clearing pItem pointer)\n");
#endif
	}
	...
	if (IsTask(TASK_DONTMOVE) && (wpt_wait_time < gpGlobals->time) && !IsWaypoint(curr_wpt_index, wpt_sniper) &&
		(pBotEnemy == NULL))
	{
...
#ifdef _DEBUG
		if (debug_actions)
			ALERT(at_console, "Now moving again (dontmove cleared)\n");
#endif
	}
...
#ifdef _DEBUG
		if (debug_actions)
			ALERT(at_console, "<%s>Cleaned open door actions\n", name);
#endif
	}
...
#ifdef _DEBUG
		if (debug_actions)
			ALERT(at_console, "***bot is no more drowning (drowning flag cleared)\n");
#endif
	}
...
#ifdef _DEBUG
		if ((debug_actions) || (debug_stuck))
			ALERT(at_console, "***resetting suicide timer/counter (bot is no more stuck)\n");
#endif
	}
	...
	// is the bot still in duckjump
	if (f_duckjump_time > gpGlobals->time)
	{
		pEdict->v.button |= IN_DUCK;		// so keep the crouch pressed
	}
	...
	if ((crowded_wpt_index != -1) && !IsTask(TASK_SPRINT) && (move_speed > SPEED_SLOW) &&
		!IsWaypoint(crowded_wpt_index, wpt_crouch) && !IsWaypoint(crowded_wpt_index, wpt_prone))
	{
	...
	if (g_debug_bot_on)
	{
		...
		if ((f_shoot_time > gpGlobals->time) &&
			((weapon_action == W_TAKEOTHER) || (weapon_action == W_INCHANGE)))
		{
